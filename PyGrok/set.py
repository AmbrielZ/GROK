...
class Set(object):
    def __init__(self, node: Node) -> None:
        ...
        node.rcolor('b')
        self._hrm = msg(head())
        self._hrm.sendto(act(1, 0).push("set"))
        # 基于传入的根节点实例化红黑树
    ...
    def insert(self, node: Node) -> None:
        ...
        self.__insertBalance(node)
        self._hrm.sendto(act(5, 1).push(node.value))
    ...
    def erase(self, value):
        ...
        self._hrm.sendto(act(6, 1).push(dnode.value))
...
def mid(root: Node):
    ...
    if root.father:
        ...
        f = None
    print(root.value, root.color, f, left, right)
    mid(root.right)
