...
test_path = 'media/videos/visual_unionfindset/480p15/50669_1.mp4'
class GrokFormMsg:
...
mob_to_manim = {
    "scapegoat": "visual_scapegoat",
    "vector": "visual_arr",
    "Heap": "visual_tarray_heap",
    "stack": "visual_stack",
    "queue": "visual_queue_fade",
    "unionfindset": "visual_unionfindset",
    "SegmentTree": "visual_segmenttree",
    "Bst": "visual_bst",
    "rbt": "visual_rbt"
}
        ...
        while 1:
            ...
            if grok_inputs[_pro_id].get(_g_id) is None:
                ...
                if grok_inputs[_pro_id][_g_id].push(__msg_) == -1:
                    ...
                    video_path = 'media/videos/'+ mob_to_manim[ss] +'/'
                    if self.level == 'm':
                    ...
                    comline = "manim -pq" + self.level +\
                              " ../datastruct/" + mob_to_manim[ss] + ".py -o " + filename
                    os.system(comline)
...
class MainWin(QMainWindow):
    ...
    def add_test(self, data):
        ...
        p = QPushButton()
        p.setText(str(test_val))
        

        test_val += 1

        self.ui.mylayout.insertWidget(self.tail, p)
        self.tail += 1
...
class V_doConfig:
    ...
    def show_pic_back(self):
        if self.flag:
            ...
            self.current_f = self.cap.get(cv2.CAP_PROP_POS_FRAMES)
            # 设置下一次帧为当前帧-20
            self.cap._set(cv2.CAP_PROP_POS_FRAMES, self.current_f - 20)
            # 读取一帧
            success, frame = self.cap.read()
            if success:
                show = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    ...
    def change_img(self):
        if self.flag:
            ...
            self.cap._set(cv2.CAP_PROP_POS_FRAMES, self.current_f)
            # 读取一帧
            success, frame = self.cap.read()
            if success:
                show = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
